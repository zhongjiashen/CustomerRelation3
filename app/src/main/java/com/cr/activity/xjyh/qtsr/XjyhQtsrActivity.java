...
import com.cr.adapter.xjyh.qtsr.XjyhQtsrAdapter;
 ...
 * 
 */
public class XjyhQtsrActivity extends BaseActivity implements OnClickListener {
	private XjyhQtsrAdapter adapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_xjyh_qtsr);
		addFHMethod();
		initActivity();
		initListView();
	...
	private void initListView() {
		...
		adapter = new XjyhQtsrAdapter(list, this);
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				Intent intent = new Intent(mContext,
						XjyhQtsrDetailActivity.class);
				intent.putExtra("billid", list.get(arg2 - 1).get("billid")
						.toString());
				if (XjyhQtsrActivity.this.getIntent().hasExtra("select")) {// 如果是添加订单时候关联的操作
	...
	public void onClick(View arg0) {
		...
		switch (arg0.getId()) {
		case R.id.sx:
			intent.setClass(mContext, XjyhQtsrSearchActivity.class);
		...
		case R.id.xz:
			...
			intent.setClass(mContext, XjyhQtsrAddActivity.class);
			startActivityForResult(intent, 1);
			break;
		}
	}
