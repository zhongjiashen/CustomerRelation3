...
import com.cr.adapter.xjyh.fyzc.XjyhFyzcAdapter;
import com.cr.adapter.xjyh.yhcq.XjyhYhcqAdapter;
 ...
 * 
 */
public class XjyhYhcqActivity extends BaseActivity implements OnClickListener {
	private XjyhYhcqAdapter adapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_xjyh_yhcq);
		addFHMethod();
		initActivity();
		initListView();
	...
	private void initListView() {
		...
		adapter = new XjyhYhcqAdapter(list, this);
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				Intent intent = new Intent(mContext,
						XjyhYhcqDetailActivity.class);
				intent.putExtra("billid", list.get(arg2 - 1).get("billid")
						.toString());
				if (XjyhYhcqActivity.this.getIntent().hasExtra("select")) {// 如果是添加订单时候关联的操作
	...
	public void onClick(View arg0) {
		...
		switch (arg0.getId()) {
		case R.id.sx:
			intent.setClass(mContext, XjyhYhcqSearchActivity.class);
		...
		case R.id.xz:
			...
			intent.setClass(mContext, XjyhYhcqAddActivity.class);
			startActivityForResult(intent, 1);
			break;
		}
	}
