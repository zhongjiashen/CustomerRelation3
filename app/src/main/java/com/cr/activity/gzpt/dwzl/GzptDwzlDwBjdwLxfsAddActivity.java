...
/**
 * 工作平台-单位资料-单位-编辑单位-联系方式-添加/修改
 * 
 * @author Administrator
 * 
 */
public class GzptDwzlDwBjdwLxfsAddActivity extends BaseActivity implements OnClickListener {
    ...
    private void initActivity() {
        ...
        lxfsSpinner.setOnItemSelectedListener(new OnItemSelectedListener() {
            ...
            public void onNothingSelected(AdapterView<?> arg0) {
            	String dhhm=activity.getIntent().getExtras().getString("dhhm");
            	if(dhhm.length()==11){
            		lxfsSpinner.setSelection(1,true);
            	}
        ...
        if(this.getIntent().hasExtra("dhhm")){
        	String dhhm=this.getIntent().getExtras().getString("dhhm");
        	dhEditText.setText(dhhm);
