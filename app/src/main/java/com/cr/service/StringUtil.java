 ...
 * 
 * @author wumi
 * @version $Id: StringUtil.java, v 0.1 2011-6-3 上午09:43:01 wumi Exp $
 */
public class StringUtil {
     ...
     * 字符串的累加操作
     * 
     * @param strs 需要累加的字符串数组
     * @return 累加后的字符串
     */
    public static String add(String... strs) {
        StringBuilder re = new StringBuilder();
        for (String ele : strs) {
            re.append(StringUtil.defaultIfEmpty(ele));
        }
        return re.toString();
    }
     ...
     * @return 累加后的字符串
     */
    public static String add(Object... strs) {
        StringBuilder re = new StringBuilder();
        for (Object ele : strs) {
            re.append(ele == null ? "" : ele);
        }
        return re.toString();
    }
    /**
     * 将十进制数转换为8位十六进制的存储格式即翻转格式
     * 
     * @param a
     * @return
     */
    public static String intTo8Hex(int a)
    {
        String temp =Integer.toHexString(a);
        StringBuffer hexStr = new StringBuffer();
        while(hexStr.length()+temp.length()<8)
        {
            hexStr.append("0");
        }
        hexStr.append(temp);
        System.out.println(hexStr);
        String outStr =hexStr.substring(6,8)+hexStr.substring(4,6)+hexStr.substring(2,4)+hexStr.substring(0,2);
